      ...
      <div className="max-w-[1120px] mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-8">
          {projects.map((project, idx) => (
            <div key={idx} className="glass-card rounded-xl p-6 flex flex-col h-full group cursor-pointer transition-all hover:-translate-y-1">
              <div className="mb-2">
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {project.tags.map((tag, tagIdx) => (
                  <span key={tagIdx} className="bg-surface-elevated text-text-secondary font-mono text-[13px] px-2 py-1 rounded border border-border">
                    {tag}
                  </span>
                ))}
              </div>
